...
description: Orquestra análise IFC no OpenClaw com agentes isolados para inventário, classes IFC, relações, IDS, bSDD, BCF e consolidação, além de orientar Revit-IFC e ISO 19650. Usar ao inspecionar arquivos IFC, distribuir análises por classe, verificar requisitos, pesquisar conceitos buildingSMART, produzir relatórios de cobertura ou registrar não conformidades. Exigir evidência determinística e revisão humana antes de alterar modelos, publicar dados ou declarar conformidade.
---
...
3. Acionar `ifc-relations` para verificações que atravessam classes.
4. Acionar `ids-validator`, `bsdd-researcher` e `bcf-coordinator` somente quando aplicáveis.
5. Acionar `ifc-consolidator` para cobertura, deduplicação e relatório final.
6. Manter o isolamento padrão de sessão do `sessions_spawn` e enviar tarefas autocontidas. Usar `sessions_yield` após os spawns; não fazer polling.
